...
class cropping():
    def __init__(self,crop_tuple):
        self.crop_tuple = crop_tuple

    def crop_bounding_box(self, image):
        return crop(image, self.crop_tuple[0], self.crop_tuple[1], self.crop_tuple[2], self.crop_tuple[3])
...
def plot_exact_point(resize, refinement_model, x, im, color=(128, 0, 128), label=None, line_thickness=3, refinement=False):
    # for box_idx in range(n):
                    ...
                    # print(temp_pil)
                    # temp_cropped = te.crop()
                    # temp_pil/=255.0
    crop_tuple = (x[0].cpu().item(),x[1].cpu().item(),x[2].cpu().item(),x[3].cpu().item())
    # print(crop_tuple)
    # print(crop_tuple)
    # temp_cropped = temp_pil.crop(crop_tuple)
    # temp_cropped_to_tensor = T.PILToTensor()(temp
    # _cropped).unsqueeze(0)
    crop_size = torch.Tensor([x[2]-x[0],x[3]-x[1]])
    # print(crop_size)
    this_cropping = cropping(crop_tuple)
    PIL_im = Image.fromarray(im.astype('uint8'), 'RGB')
    # print(PIL_im)
    PIL_im_crop = PIL_im.crop(crop_tuple)
    ...
    with torch.no_grad():
        ...
        # print(refinement_output)
    refine_estimation = ([crop_tuple[0]+refinement_output[0],crop_tuple[1]+refinement_output[1]])
